...
export async function saveLibraryAnalysis({ analysis, itemCount }) {
  // Always keep single row — delete old, insert new
  await supabase.from('library_analysis').delete().neq('id', '00000000-0000-0000-0000-000000000000');
  const { data, error } = await supabase
    .from('library_analysis')
    .insert([{ analysis, item_count: itemCount, updated_at: new Date().toISOString() }])
    .select()
    .single();
  if (error) console.error('Library analysis save error:', error);
  return data;
}

export async function loadLibraryAnalysis() {
  const { data, error } = await supabase
    .from('library_analysis')
    .select('*')
    .order('updated_at', { ascending: false })
    .limit(1)
    .maybeSingle();
  if (error) console.error('Library analysis load error:', error);
  return data;
}
