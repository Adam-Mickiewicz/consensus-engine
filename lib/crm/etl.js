...
export function flattenShoperCSV(rows) {
  ...
  for (const row of rows) {
    ...
    for (let i = 1; i <= 59; i++) {
      ...
      // EAN / product_code z Shopera
      const codeKey    = `product_code ${i}`;
      const altCodeKey = `kod produktu ${i}`;
      const eanRaw     = row[codeKey] ?? row[altCodeKey] ?? null;
...
export async function mapWorldsAndTaxonomy(lineItems, supabase) {
  const { data: products, error } = await supabase
    .from("products")
    .select("ean, name, tags_granularne, tags_domenowe, filary_marki, okazje, segment_prezentowy, launch_date, evergreen, created_at");

  if (error) throw new Error(`products fetch error: ${error.message}`);

  const productList = products ?? [];

  // Mapa EAN → product dla szybkiego lookup po EAN
  const eanMap = new Map();
  for (const p of productList) {
    if (p.ean) eanMap.set(Number(p.ean), p);
  }

  // Fallback: indeks nazw (lowercase) dla starych danych bez EAN
  const productIndex = productList.map((p) => ({
    ...p,
    _nameLower: (p.name ?? "").toLowerCase(),
  }));

  function applyProduct(li, match) {
    return {
      ...li,
      ean: match.ean,
      ...
      launch_date: match.launch_date ?? null,
  ...
  return lineItems.map((li) => {
    ...
    // Strategia 1: match po EAN (product_code z Shopera)
    if (li.ean) {
      const match = eanMap.get(Number(li.ean));
      if (match) return applyProduct(li, match);
    }

    // Strategia 2: fallback — text match po nazwie (dla starych danych bez EAN)
...
export async function assignPromoAndNewProduct(lineItems, supabase) {
  ...
  return lineItems.map((li) => {
    ...
    // is_new_product: zamówienie w ciągu 90 dni od launch_date produktu
    // Fallback: products.created_at jeśli launch_date niedostępne
    let is_new_product = false;
    const launchRef = li.launch_date ?? li.product_created_at;
    if (launchRef && li.date) {
      const diffDays =
        (new Date(li.date).getTime() - new Date(launchRef).getTime()) /
        (86400 * 1000);
...
export async function upsertToSupabase(clients, lineItems, supabase) {
  ...
  const eventRows = lineItems
    ...
    .map((li) => ({
      ...
      promo_code:        li.promo_code ?? null,
      discount_code:     li.discount_code ?? 0,
      discount_client:   li.discount_client ?? 0,
      shipping_cost:     li.shipping_cost ?? 0,
    }));
  ...
  for (let i = 0; i < eventRows.length; i += UPSERT_BATCH) {
    ...
    if (error) {
      console.error(`[upsert client_product_events] batch ${i}-${i + batch.length} error:`, JSON.stringify(error, null, 2));
      throw new Error(`upsert client_product_events error: ${error.message} (code: ${error.code}, details: ${error.details}, hint: ${error.hint})`);
    }
  }

  const { count: evCount } = await supabase.from("client_product_events").select("*", { count: "exact", head: true });
  console.log("[DEBUG] events w bazie po upsert:", evCount);
    ...
    lineItems = rawOrders.flatMap((order) =>
      order.products.length > 0
        ? order.products.map((p) => ({
            ...
            discount_code:   order.discount_code ?? 0,
            discount_client: order.discount_client ?? 0,
