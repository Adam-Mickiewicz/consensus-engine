...
/**
 * Przelicza LTV dla podanych klientów bezpośrednio z client_product_events.
 * Nadpisuje clients_360.ltv sumą line_total ze wszystkich eventów w DB.
 */
async function recalculateLTV(supabase, clientIds) {
  ...
  if (error) throw new Error(`recalculateLTV fetch error: ${error.message}`);

  const ltvMap = {};
  for (const id of clientIds) ltvMap[id] = 0;
  ...
  for (let i = 0; i < updates.length; i += UPSERT_BATCH) {
    ...
    if (upErr) throw new Error(`recalculateLTV upsert error: ${upErr.message}`);
  }
}
...
export async function upsertToSupabase(clients, lineItems, supabase) {
  ...
  // Przelicz LTV z DB (SUM line_total) po wstawieniu eventów — nadpisuje
  // wartość z ETL, która obejmowała tylko bieżący import, nie pełną historię.
  const affectedClientIds = clientRows.map((r) => r.client_id);
  await recalculateLTV(supabase, affectedClientIds);
