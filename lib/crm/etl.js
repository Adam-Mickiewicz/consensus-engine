...
export async function anonymizeAndVault(lineItems, supabase) {
  ...
  for (const email of uniqueEmails) {
    ...
    if (existingMap.has(hash)) {
      ...
      // email jest przechowywany obok hasha wyłącznie do celów eksportu (edrone CSV)
      // Dostęp do master_key wymaga service_role — nigdy nie eksponuj przez publiczne API.
      newVaultEntries.push({ email_hash: hash, client_id: clientId, email });
    }
  }
