...
export async function anonymizeAndVault(lineItems, supabase) {
  ...
  // Pobierz istniejące wpisy z master_key (batch)
...
export async function upsertToSupabase(clients, lineItems, supabase) {
  ...
  const clientRows = clients.map((c) => ({
    ...
    ulubiony_swiat: c.ulubiony_swiat,
    worlds_list: c.worlds_list,
    events_list: c.events_list,
    purchase_frequency_yearly: c.purchase_frequency_yearly,
    full_order_history: c.full_order_history,
...
export async function runETLPipeline(rawOrders, supabase, sourceFile = "api") {
  ...
  // Kroki 2–7
  const withClientIds = await anonymizeAndVault(lineItems, supabase);
  const withTaxonomy  = await mapWorldsAndTaxonomy(withClientIds, supabase);
  const withOccasions = assignOccasion(withTaxonomy);

  const unmapped = withOccasions.filter((li) => li._mapped === false).length;

  const profiles   = buildClientProfiles(withOccasions);
  const segmented  = segmentClients(profiles);
  const result     = await upsertToSupabase(segmented, withOccasions, supabase);
