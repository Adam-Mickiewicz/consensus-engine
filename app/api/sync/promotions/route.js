...
function mapRow(row) {
  return {
    ...
    discount_min:   row.discount_min    ?? row["Rabat min"]    ?? row["discount_value_min"] ?? null,
    discount_max:   row.discount_max    ?? row["Rabat max"]    ?? row["discount_value_max"] ?? null,
...
export async function POST(request) {
  ...
  try {
    ...
    const { error } = await supabase
      .from("promotions")
      .upsert(mapped, { onConflict: "promo_name,start_date", ignoreDuplicates: false });
