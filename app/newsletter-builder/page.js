...
function generateProductsHTML(products) {
  const renderCard = (p) => {
    if (!p) return `<td width="50%" valign="top" style="padding:0 4px;"></td>`;
    ...
    return `<td width="50%" valign="top" style="padding:0 4px;">
      ...
      <div style="background:#ffffff; border-radius:0 0 5px 5px; padding:6px 6px 8px 6px;">
        <p style="font-family:'Playfair Display',serif; font-size:12px; color:#000; margin:0 0 2px 0; line-height:1.4; height:42px; overflow:hidden;">${name}</p>
        <div style="min-height:42px; max-height:42px; overflow:hidden;">
  ...
  for (let i = 0; i < products.length; i += 2) {
    rows.push(`<table border="0" cellpadding="0" cellspacing="0" width="100%" style="max-width:600px; margin-bottom:8px;">
  <tr>
    ${renderCard(products[i])}
    ${renderCard(products[i + 1] || null)}
  </tr>
</table>`);
  }
