...
export default async function OccasionsPage() {
  ...
  const [retentionRes, firstRes, ltvRes, loyalCountRes, loyalRes, driftRes] =
    await Promise.all([
      ...
      sb.from('crm_occasion_loyal').select('*', { count: 'exact', head: true }),
      sb.from('crm_occasion_loyal').select('*').order('years_active', { ascending: false }).order('total_ltv', { ascending: false }).limit(50),
      sb.from('crm_occasion_drift').select('*').order('occasion_count', { ascending: false }).limit(50),
    ]);
