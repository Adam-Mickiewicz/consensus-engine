...
import { overview } from "../../../../lib/crm/mockData";
...
export default function CrmAnalyticsPage() {
  ...
  const maxWorld = Math.max(...overview.topWorlds.map(w => w.count));
  const maxRisk = Math.max(...overview.byRisk.map(r => r.count));
      ...
      <div className="oa-wrap">
        ...
        <div className="oa-kpis">
          <div className="oa-kpi">
            <div className="oa-kpi-val">{overview.totalCustomers.toLocaleString('pl-PL')}</div>
            <div className="oa-kpi-label">Łączna liczba klientów</div>
          </div>
          <div className="oa-kpi">
            <div className="oa-kpi-val oa-kpi-accent">{fmtPln(overview.totalLtv)}</div>
            <div className="oa-kpi-label">LTV łączne (suma)</div>
          </div>
          <div className="oa-kpi">
            <div className="oa-kpi-val">{overview.avgLtv.toLocaleString('pl-PL')} zł</div>
            <div className="oa-kpi-label">Średnie LTV na klienta</div>
          </div>
          <div className="oa-kpi">
            <div className="oa-kpi-val" style={{ color: "#f97316" }}>{overview.vipReanimacja}</div>
