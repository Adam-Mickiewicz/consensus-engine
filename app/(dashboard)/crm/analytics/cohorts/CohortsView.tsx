...
export type CohortsData = {
  matrix: { cohort: string; size: number; m1ret: number }[];
  avgTimeToSecond: { segment: string; days: number | null }[];
  retentionBySegment: { segment: string; total: number; repeat: number; rate: number }[];
} | null;

export default function CohortsView({ data }: { data: CohortsData }) {
  ...
  const displayMatrix = data.matrix.slice(-18);
      ...
      <div className="ch-wrap">
        ...
        <div className="ch-block">
          <div className="ch-section">
            Macierz kohort — retencja miesięczna (ostatnie 18 kohort, % klientów z 2+ zakupami)
          </div>
          <div className="ch-card">
                ...
                <tbody>
                  {displayMatrix.map(row => (
                    <tr key={row.cohort}>
                      ...
                      <td className="cohort-0">100%</td>
                      <td style={{
                        background: retColor(row.m1ret),
                        color: row.m1ret === 0 ? t.border : t.text,
                      }}>
                        {row.m1ret > 0 ? `${row.m1ret}%` : "—"}
                      </td>
                    </tr>
                  ))}
                </tbody>
