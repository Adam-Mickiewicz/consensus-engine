...
export default function MarketingBrief() {
  ...
  const openNew = () => { setBrief(defaultBrief()); setEditId(null); setActiveChannel(null); setView("form"); setFormTab("brief"); };
  const openEdit = (b) => {
    setBrief(b.data);
    setEditId(b.id);
      ...
      if (brief.priority >= 4) {
        ...
        children.push(new Table({
          ...
          rows: [new TableRow({ children: [
            new TableCell({
              borders: noBorders,
              shading: { fill: priorityColor, type: ShadingType.CLEAR },
              margins: { top: 200, bottom: 200, left: 200, right: 200 },
              width: { size: PAGE_W, type: WidthType.DXA },
              children: [new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: priorityText, bold: true, size: 32, font: "Arial", color: "FFFFFF" })] })]
            })
          ]})]
        }));
