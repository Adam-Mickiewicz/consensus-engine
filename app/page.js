...
const CONSENSUS_PROMPT = (all, detail) => `You are the Consensus Builder. Here are revised proposals from three AI experts: ${all} Build the BEST POSSIBLE consensus. CRITICAL: Start final_recommendation with a clear YES, NO, or CONDITIONAL answer in one sentence. Then explain why. Only then provide details. ${detail} Use rich markdown formatting. Respond ONLY in valid JSON: final_recommendation (string), rationale (string), agreed_points (array of strings), disputed_but_resolved_points (array of strings), unresolved_questions (array of strings), action_plan (array of strings), confidence_score (number 0-100). ${LANG}`;
