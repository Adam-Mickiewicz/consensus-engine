...
const CONSENSUS_PROMPT = (all, detail) => `You are the Consensus Builder. Here are revised proposals: ${all} Build consensus. ${detail} OUTPUT RULES: Single-line JSON, no newlines, ALL string values max 100 words, arrays max 3 items each max 15 words. Fields: final_recommendation (string), rationale (string), agreed_points (array of max 3 short strings), disputed_but_resolved_points (array of max 3 short strings), unresolved_questions (array of max 2 short strings), action_plan (array of max 3 short strings), confidence_score (number). ${LANG}`;
      ...
      <div style={{ borderRight: `1px solid ${t.border}`, padding: 24, overflowY: "auto", background: t.bgPanel, ...(isMobile ? { position: "fixed", top: 0, left: sidebarOpen ? 0 : "-280px", width: 260, height: "100vh", zIndex: 999, transition: "left 0.3s", boxShadow: sidebarOpen ? "4px 0 20px rgba(0,0,0,0.2)" : "none" } : {}) }}>
        <div style={{ marginBottom: 28, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <div style={{ marginTop: 8, paddingTop: 8, borderTop: "1px solid " + t.border }}>
            {user ? (<div><div style={{ color: t.textMuted, fontSize: 10, marginBottom: 4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{"Logged in: " + user.email}</div><button onClick={async () => { const m = await import("../lib/supabase"); await m.supabase.auth.signOut(); setUser(null); }} style={{ width: "100%", background: "none", border: "1px solid " + t.border, borderRadius: 8, padding: "7px 10px", cursor: "pointer", color: t.textSub, fontSize: 11, fontFamily: "inherit" }}>Wyloguj</button></div>) : (<button onClick={() => window.location.href = "/auth"} style={{ width: "100%", background: accent + "15", border: "1px solid " + accent + "40", borderRadius: 8, padding: "7px 10px", cursor: "pointer", color: accent, fontSize: 11, fontFamily: "inherit", fontWeight: 700 }}>Zaloguj sie</button>)}
          </div>
        </div>
